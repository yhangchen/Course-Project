...
def load_mnist():
    ...
    print(
        '\n----- MNIST datasets loaded: {:d} training samples and {:d} test samples.'
        .format(data['A_train'].shape[0], data['A_test'].shape[0]))
...
def get_operators_based_on_data(A_train, b_train, b_train_binarized):
    ...
    stocgradfx = lambda Y, index: operators.stocgradfx(Y, index, A_train,
                                                       b_train_binarized)
...
def print_progress(i, maxit, val_F, val_f, val_g):
    print('\n--- iter = {:d}/{:d}, F(X) = {:f}, f(X) = {:f}, g(X) = {:f}.'.
          format(i, maxit, val_F, val_f, val_g))
