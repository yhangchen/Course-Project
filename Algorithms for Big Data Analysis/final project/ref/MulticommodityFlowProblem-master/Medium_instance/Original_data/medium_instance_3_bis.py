...
#   -------- DIJKSTRA'S ALGORITHM --------
    to_Disconnect = list(mostUsedNode)
    if(len(to_Disconnect) != 0):
        for j in range(len(to_Disconnect)):
            for i in range(nStations):
                cost[i, to_Disconnect[j], 0] = 0 # Disconnect the to_Disconnect[j] from all others (cumulative)
...
def getMostUsedNode(): 
    ...
    print('Node with exeeced value SET : ', set(np.nonzero(count_node>0)[0].tolist()))
    print('EXCEEDING : ',np.amax(count_node)) # Most violated node
    print('will be deleted :', count_node.argsort()[-3:][::-1].tolist()) #Take the 3 most violated nodes
    
    return count_node.argsort()[-3:][::-1].tolist()
...
capacity_node = [50, 85, 130, 180] # Define a capacity value for each type of node
    ...
    while True:
        ...
        if(obj_Function > -1):  # We found a feasible solution
            if(initFind == False): # Just to set the timer correctly
                ...
                initFind = True
            break
        to_Remove = set(getMostUsedNode())
        
        mostUsedNode = mostUsedNode.union(to_Remove) #Compute the most violated node
        usedNode = usedNode.union(mostUsedNode)
