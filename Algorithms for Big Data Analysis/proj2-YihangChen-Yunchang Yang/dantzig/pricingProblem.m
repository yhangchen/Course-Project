function [reducedCost, bestPath_nodes, forCommodity, bestPath_arcs]=pricingProblem(dualCommodities, dualStations, dualArcs, nStations, node, nArcs, arc, nCommodities, commodities, cost)
...
shortest_from_node = inf(nStations, nStations); % Total cost of the shortest path from index
...
for i=1:length(dualArcs)
    if cost_for_commo(arc(i,1),arc(i,2),1)>0
        cost_for_commo(arc(i,1),arc(i,2),1) = dualArcs(i);
    end
end
end
...
for i=1:nCommodities
    ...
    if isequal(previous_nodes(origin,:),ones(1,nStations)*-1) % Haven't run Dijkstra yet for this origin
        [shortest_from_node(origin,:), previous_nodes(origin,:), previous_arcs(origin,:)] = dijkstra(origin, cost_for_commo, [], [],nStations, arc);
    end
    
    currentReducedCost = (shortest_from_node(origin, dest) * commodities(i,3)) + dualCommodities(i); %Compute the reducedCost
    reducedCost = min(reducedCost,currentReducedCost);
    if currentReducedCost<0
        ...
        while dest~=origin
            ...
            newPath_nodes(nStations+1)=newPath_nodes(nStations+1)+arc(previous_arcs(origin,dest),3);
